document.addEventListener('ProjectValidated', async (e) => {
    ...
    if (userRole === 'PM' || userRole === 'DEV') {
        const reportBtn = document.getElementById('reportDefectBtn');
    ...
    async function loadModulesForSelect() {
        try {
            ...
            if (res.ok) {
                ...
                const projectModules = allModules.filter(mod => mod.project === parseInt(PROJECT_ID));
    ...
    function getSeverityBadge(severity) {
        switch(severity) {
            case 'Critical': return '<span class="badge text-bg-danger"><i class="fa-solid fa-fire me-1"></i>Critical</span>';
            case 'High': return '<span class="badge text-bg-warning"><i class="fa-solid fa-arrow-up me-1"></i>High</span>';
            case 'Medium': return '<span class="badge text-bg-info text-dark">Medium</span>';
            case 'Low': return '<span class="badge text-bg-secondary"><i class="fa-solid fa-arrow-down me-1"></i>Low</span>';
    ...
    async function loadDefects() {
        ...
        let url = `/api/defects/defects/?project=${PROJECT_ID}&ordering=${ordering}`;
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (defects.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="7" class="text-center py-4 text-secondary">No defects found matching criteria in this project.</td></tr>';
                    return;
                }

                defects.forEach(d => {
                    const riskStyle = d.calculated_risk > 60 ? 'color: #f85149; font-weight: bold;' : 'color: #c9d1d9;';

                    tbody.innerHTML += `
                        <tr style="border-bottom: 1px solid #30363d;">
                            <td class="ps-3 py-3">
                                <span class="text-secondary" style="font-size:12px;">#${d.id}</span><br>
                                <strong style="color: #c9d1d9;">${d.title}</strong>
                            </td>
                            <td><span class="badge" style="background-color: #21262d; border: 1px solid #30363d;">${d.module_name}</span></td>
                            <td>${getStatusBadge(d.status)}</td>
                            <td>${getSeverityBadge(d.severity)}</td>
                            <td><span style="${riskStyle}">${d.calculated_risk.toFixed(1)}</span></td>
                            <td style="font-size: 13px; color: #8b949e;">@${d.reporter_username}</td>
                            <td class="text-end pe-3">
                                <a href="/defects/${d.id}/" class="btn btn-sm btn-gh-secondary">Details</a>
                            </td>
                        </tr>
                    `;
                });
    ...
    document.getElementById('createDefectForm')?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                bootstrap.Modal.getInstance(document.getElementById('createDefectModal')).hide();
                document.getElementById('createDefectForm').reset();
                loadDefects();
                Swal.fire({ icon: 'success', title: 'Defect Created', background: '#161b22', color: '#c9d1d9'});
            }
        } catch (error) { console.error(error); }
    });
